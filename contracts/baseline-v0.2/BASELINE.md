# ClawLoops 冻结基线（baseline-v0.2）

冻结目标：把 **字段、状态机、错误码、接口边界** 定死，后续任何改动必须遵守“兼容性规则/变更流程”，避免反复返工。

- **基线版本**：v0.2
- **冻结日期**：2026-03-12
- **权威来源（本次已对齐）**：`/workspace/MVP_Contract.md`、`/workspace/Architecture_Design.md`、`/workspace/API_Spec.md`
...
除 `GET /api/v1/auth/me` 外：disabled 用户访问业务接口统一返回 `403 USER_DISABLED`，不得再用 `ready=false` 的变体绕开收口。

## 6. 兼容性规则与变更流程（冻结后的唯一改法）

### 6.1 允许的向后兼容变更（不破坏 v0.2）
